...
<body class="flex flex-col items-center h-screen" x-data="validateForm">
    ...
    <template x-if="location!=null">
        <p
            @click="console.log(location.data);console.log(`https://www.google.com/maps/embed/v1/place?key=AIzaSyD3B5J-Vd0i6dDCsOQ2YRnl9ttE7tnd2Ds
    ...
    <section class="form w-2/4 h-4/5 flex flex-col justify-center">
        ...
        <div x-show="step === 1" class="step-1 flex flex-col mt-5 mb-5">
            <input x-model="formData.firstName" @change="checkText('firstName',length.nameLength)"
                class="border-b-2 border-b-rose-100 mb-5 h-8" type="text" name="first_name" id="first-name"
                placeholder="First Name">
            <span x-text="errors.firstNameError"></span>
            <input x-model="formData.lastName" @change="checkText('lastName', length.nameLength)"
                class="border-b-2 border-b-rose-100 mb-5 h-8" type="text" name="last-name" id="last-name"
                placeholder="Last Name">
            <span x-text="errors.lastNameError"></span>

            <input x-data x-model="formData.phoneNumber" x-mask="+99(999)999-999" @change="checkNumber()"
                placeholder="+NN (NNN) NNN-NNNN" class="border-b-2 border-b-rose-100 mb-5 h-8" type="text" name="phone"
                id="phone" placeholder="Number">
            <span x-text="errors.phoneNumberError"></span>

            <input x-model="formData.email" @change="checkEmail()" class="border-b-2 border-b-rose-100 mb-5 h-8" type="email" name="email" id="email"
                placeholder="Email">
            <span x-text="errors.emailError"></span>

            <select x-model="formData.country" @change="checkCountry()" class="border-b-2 border-b-rose-100 mb-5 h-8"
            ...
            <input  @change="uploadImg($event.target.files)"
                class="block file:rounded-lg file:border-none file:mr-5 file:bg-rose-400 file:text-white file:cursor-pointer file:w-4/12  file:h-10 h-10 cursor-pointer mb-5"
                type="file" accept="image/*" name="user-img" id="user-img">
            <span id="img" x-text = "errors.imgError"> </span>
        </div>

        <div x-show="step === 2" class="step-2 flex flex-col mt-5 mb-5">
            <input 
            x-model="formData.topic" @change="checkText('topic',length.topicLength)"
            class="border-b-2 border-b-rose-100 mb-5 h-8" type="text" name="topic" id="topic"
                placeholder="Your Topic">
            <span x-text="errors.topicError"> </span>
 
            <textarea
            x-model="formData.description" @change="checkText('description', length.descriptionLength,1000)"
            class="border-b-2 border-b-rose-100 mb-5" name="description" id=""></textarea>
            <span x-text="errors.descriptionError"> </span>
           
            <input
            x-model="formData.date" @change="checkDate()"
            :min="minDate"
            class="border-b-2 border-b-rose-100 mb-5" type="date" name="date" id="date">
